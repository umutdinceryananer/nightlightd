...
# The tray target needs Xephyr too, and hands off to scripts/tray-menu.sh.
...
#   scripts/screenshots.sh tray       # the tray menu, via scripts/tray-menu.sh
set -euo pipefail
...
case "${1:-all}" in
    ...
    panel)
        ...
        panel_shot nord  settings panel-settings.png
        ;;
    # Its own script, and a long way from the rest of this one: the menu
    # cannot be photographed in the session you are sitting in — the click
    # that opens it also toggles your filter — so it is done inside a
    # throwaway desktop. See scripts/tray-menu.sh (#51).
    tray) exec "$repo/scripts/tray-menu.sh" ;;
    *)
